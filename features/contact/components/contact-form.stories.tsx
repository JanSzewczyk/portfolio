...
ContactForm_.test(
  "Renders all form fields and submit button",
  async ({ canvas }) => {
...
ContactForm_.test(
  "Successfully submits valid form data",
...
ContactForm_.test(
  ...
  async ({ canvas, step, userEvent }) => {
    ...
    await step("Verify success state is shown", async () => {
      await waitFor(
        async () => {
          const thankYouMessage = canvas.getByText(
...
ContactForm_.test(
  ...
  async ({ canvas, args, step, userEvent }) => {
    await step("First submission", async () => {
      ...
      await userEvent.type(
        messageInput,
        "This is the first test message that is long enough",
      );
      await userEvent.click(submitButton);
    ...
    await step("Second submission with new data", async () => {
      ...
      await userEvent.type(
        messageInput,
        "This is the second test message that should also work fine",
      );
...
ContactForm_.test(
  "Validates email field correctly",
  async ({ canvas, args, step, userEvent }) => {
    const emailInput = canvas.getByLabelText(/email/i);
    const nameInput = canvas.getByLabelText(/username/i);
    const messageInput = canvas.getByLabelText(/message/i);
...
ContactForm_.test(
  "Validates message field correctly",
  async ({ canvas, step, userEvent }) => {
    const messageInput = canvas.getByLabelText(/message/i);
    const submitButton = canvas.getByRole("button", { name: /send message/i });
...
ContactForm_.test(
  ...
  async ({ canvas, args, step, userEvent }) => {
    ...
    await step("Wait for submission to complete", async () => {
      await waitFor(
        async () => {
          const thankYouMessage = canvas.queryByText(
            /thank you for your message/i,
          );
          await expect(thankYouMessage).toBeVisible();
        },
...
ContactForm_.test(
  ...
  async ({ canvas, args, step, userEvent }) => {
    ...
    await step("Form should not be cleared on error", async () => {
      ...
      await expect((emailInput as HTMLInputElement).value).toBe(
        "john@example.com",
      );
    });
  },
