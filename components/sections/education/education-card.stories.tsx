...
WithThesisAndProject.test(
  "Renders institution and field of study",
  async ({ canvas }) => {
...
WithThesisAndProject.test(
  "Renders thesis section with title and description",
  async ({ canvas }) => {
    ...
    const thesisTitle = canvas.getByText(
...
WithThesisAndProject.test(
  "Renders related project with icon action buttons",
  async ({ canvas }) => {
...
WithThesisAndProject.test(
  ...
  async ({ canvas }) => {
    ...
    await expect(liveButton).toHaveAttribute(
      "href",
      "https://collabcode.example.com",
    );
    await expect(codeButton).toHaveAttribute(
      "href",
      "https://github.com/example/collabcode",
    );
    await expect(thesisButton).toHaveAttribute(
      "href",
    ...
    education: educationBuilder.one({
      overrides: {
        ...
        thesis: () => ({
          ...
          description:
            "Design of a testing framework combining unit, integration, and visual regression testing.",
          technologies: [
            technologyBuilder.one({
              overrides: { name: "React", icon: "SiReact" },
            }),
        ...
        achievements: () => [
          "Best thesis award in Software Engineering department",
        ],
        coursework: () => [
...
ThesisWithoutProject.test(
  ...
  async ({ canvas }) => {
    ...
    await expect(thesisButton).toHaveAttribute(
      "href",
      "https://example.com/bachelor-thesis.pdf",
    );
...
MinimalCard.test(
  "Does not render GPA badge when grade is null",
  async ({ canvas }) => {
...
MinimalCard.test(
  "Renders 'Present' for ongoing education",
  async ({ canvas }) => {
...
MinimalCard.test(
  "Renders institution as plain text without link",
  async ({ canvas }) => {
    ...
    education: educationBuilder.one({
      overrides: {
        ...
        thesis: () => ({
          ...
          description:
            "Optimizing neural network architectures for deployment on resource-constrained edge devices.",
          technologies: [
            technologyBuilder.one({
          ...
          project: {
            ...
            title: "EdgeNAS",
            description:
              "Neural Architecture Search toolkit optimized for edge deployment.",
            technologies: [
              technologyBuilder.one({
                overrides: { name: "Python", icon: "SiPython" },
              }),
