...
SkillsSection_Story.test(
  "Renders marquee with tech logos",
  async ({ canvas, args }) => {
...
SkillsSection_Story.test(
  "Renders category badges for technology groups",
  async ({ canvas, args }) => {
...
SkillsSection_Story.test(
  "Renders skill cards with technology names",
  async ({ canvas, args }) => {
...
SkillsSection_Story.test(
  "Renders decorative bottom text",
  async ({ canvas }) => {
    const bottomText = canvas.getByText(
      /always learning and exploring new technologies/i,
    );
...
DesktopBentoGridLayout.test(
  "Renders one card per technology group",
  async ({ canvas, args }) => {
    const groups = args.skills?.technologyGroups ?? [];
...
MobileStackedLayout.test(
  "Renders badges for technologies",
  async ({ canvas, args }) => {
    const firstGroup = args.skills?.technologyGroups?.[0];
    const firstTech = firstGroup?.technologies?.[0];
...
SectionHeadingStructure.test(
  "Skill cards use proper heading levels",
  async ({ canvas, args }) => {
...
EmptySkills.test(
  "Handles empty technology groups gracefully",
  async ({ canvas }) => {
    const heading = canvas.getByRole("heading", { level: 2 });
    await expect(heading).toBeVisible();
    ...
    const bottomText = canvas.queryByText(
      /always learning and exploring new technologies/i,
    );
