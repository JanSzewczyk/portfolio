...
// Test 1: Avatar Image
HeroSectionWithAvatar.test(
  "Renders avatar image with correct alt text",
  async ({ canvas, args }) => {
...
HeroSectionWithAvatar.test(
  "Renders available status indicator correctly",
  async ({ canvas }) => {
...
HeroSectionWithAvatar.test(
  "Renders heading with typing animation",
  async ({ canvas }) => {
...
HeroSectionWithAvatar.test(
  "Renders all alternative titles in rotation",
  async ({ canvas, args }) => {
    if (
      args.hero?.alternativeTitles &&
      args.hero.alternativeTitles.length > 0
    ) {
...
HeroSectionWithAvatar.test(
  "Get in Touch button is clickable and has correct text",
  async ({ canvas }) => {
...
HeroSectionWithAvatar.test(
  "View Projects button is clickable and has correct icon",
  async ({ canvas }) => {
...
HeroSectionWithAvatar.test(
  "Section has correct heading hierarchy",
  async ({ canvas }) => {
    const heading = canvas.getByRole("heading", { level: 1 });
    await expect(heading).toBeVisible();
...
HeroSectionWithAvatar.test(
  "Scroll indicator has proper visual feedback",
  async ({ canvasElement }) => {
...
HeroSectionWithAvatar.test(
  "GridBackground component is rendered",
  async ({ canvasElement }) => {
...
HeroSectionWithAvatar.test(
  "Container has proper layout structure",
  async ({ canvasElement }) => {
    const section = canvasElement.querySelector("section");
...
// Test 19: Avatar Size
HeroSectionWithAvatar.test(
  "Avatar has proper size styling",
  async ({ canvasElement }) => {
...
HeroSectionWithAvatar.test(
  "Name in heading uses stegaClean utility",
  async ({ canvas, args }) => {
    const heading = canvas.getByRole("heading", { level: 1 });
...
HeroSectionWithoutAvatar.test(
  "Renders initials fallback when avatar is missing",
  async ({ canvas, args }) => {
...
HeroSectionWithoutAvatar.test(
  "Renders unavailable status indicator correctly",
  async ({ canvas }) => {
...
HeroSectionWithoutAvatar.test(
  "Still renders all other content correctly without avatar",
  async ({ canvas, args }) => {
    // Verify heading
    const heading = canvas.getByRole("heading", { level: 1 });
    await expect(heading).toBeVisible();
...
HeroSectionWithoutAvatar.test(
  "Handles missing alternative titles gracefully",
  async ({ canvas }) => {
...
HeroSectionWithoutAvatar.test(
  "Handles empty alternative titles array",
  async ({ canvas }) => {
...
// Test 16: Section ID
HeroSectionWithoutAvatar.test(
  "Component renders within section with correct ID",
  async ({ canvasElement }) => {
