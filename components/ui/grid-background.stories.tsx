...
Default.test("Renders container with correct attributes", async ({ canvas, step }) => {
...
Default.test("Sets grid size via CSS custom property", async ({ canvas, step }) => {
  const container = canvas.getByTestId("grid-background-container");

  await step("Verify --grid-size custom property is set to default (40px)", async () => {
...
Default.test("Sets grid color via CSS custom property", async ({ canvas, step }) => {
  const container = canvas.getByTestId("grid-background-container");
...
Default.test("Renders fade overlay when fade is true", async ({ canvas, step }) => {
  const container = canvas.getByTestId("grid-background-container");
...
Default.test("Does not render fade overlay when fade is false", async ({ canvas, step }) => {
...
Default.test("Uses default values when props not provided", async ({ canvas, step }) => {
  const container = canvas.getByTestId("grid-background-container");
...
Default.test("Applies pointer-events-none for non-interactivity", async ({ canvas, step }) => {
  const container = canvas.getByTestId("grid-background-container");
...
Default.test("Has correct z-index for background layering", async ({ canvas, step }) => {
  const container = canvas.getByTestId("grid-background-container");
...
WithoutFade.test("Does not render fade overlay", async ({ canvas, step }) => {
  ...
  await step("Verify container has only one child div (grid only)", async () => {
    const children = container.querySelectorAll("div");
...
CustomGridColor.test("Applies custom grid color", async ({ canvas, step }) => {
  ...
  await step("Verify --grid-color custom property is set to custom value", async () => {
    const style = window.getComputedStyle(container);
    const gridColor = style.getPropertyValue("--grid-color").trim();
...
FullyCustomized.test("Applies all custom configurations", async ({ canvas, step }) => {
  const container = canvas.getByTestId("grid-background-container");
