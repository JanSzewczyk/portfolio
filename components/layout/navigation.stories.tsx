...
NavigationStory.test(
  "Renders personal name as clickable logo link",
  async ({ canvas, args }) => {
...
NavigationStory.test(
  "Semantic HTML structure is accessible",
  async ({ canvas, step }) => {
...
MobileNavigationStory.test(
  "Mobile menu button has correct ARIA label states",
  async ({ canvas, step, userEvent }) => {
    ...
    await step(
      "Click to open and verify label changes to 'Close menu'",
      async () => {
    ...
    await step(
      "Click to close and verify label returns to 'Open menu'",
      async () => {
