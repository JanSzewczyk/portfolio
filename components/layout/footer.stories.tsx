...
Default.test(
  "Renders copyright text correctly",
    ...
    await step("Verify copyright text is displayed", async () => {
      if (args.footer?.copyrightText) {
        const copyrightText = canvas.getByText(
          new RegExp(args.footer.copyrightText, "i"),
        );
...
Default.test(
  "Renders social media links with correct attributes",
  async ({ canvas, step, args }) => {
    await step("Verify social links are present", async () => {
      if (
        args.personalInfo?.socialLinks &&
        args.personalInfo.socialLinks.length > 0
      ) {
...
Default.test("Accessibility features", async ({ canvas, step, args }) => {
  ...
  await step(
    "Verify all social links are accessible via aria-label",
    async () => {
...
Default.test(
  "Keyboard navigation",
  async ({ canvas, step, userEvent, args }) => {
    if (
      !args.personalInfo?.socialLinks ||
      args.personalInfo.socialLinks.length === 0
    ) {
    ...
    await step("Tab to first social link", async () => {
      ...
      if (firstLink && firstLink.platform) {
        ...
        const isFocused =
          ...
          document.activeElement?.contains(linkElement);
    ...
    if (
      args.personalInfo?.socialLinks &&
      args.personalInfo.socialLinks.length > 1
    ) {
